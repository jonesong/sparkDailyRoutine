...
public class ProjectController {
	...
	public static Route editOneProject = (Request request, Response response) -> {
		LoginController.ensureUserIsLoggedIn(request, response);
		if (ensureUserProject(request, response)) {
			Map<String, Object> model = new HashMap<>();
	...
	public static Route editProjectPost = (Request request, Response response) -> {
		LoginController.ensureUserIsLoggedIn(request, response);
		Map<String, Object> model = new HashMap<>();
		ProjectEntity project = new ProjectEntity(getQueryProjectId(request), getQueryName(request),
				getQueryNote(request));
		project.update();
		model.put("project", project);
	...
	private static boolean ensureUserProject(Request request, Response response) {
