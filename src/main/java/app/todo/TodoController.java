...
public class TodoController {
	...
	public static Route handleTodoPost = (Request request, Response response) -> {
    	...
    	if (clientAcceptsHtml(request)) {
    		...
    		TodoEntity todo = new TodoEntity(Integer.parseInt(getSessionCurrentTaskId(request)), getQueryNote(request), getQueryDateStarted(request), getQueryTimeTotal(request), getQueryTimeStart(request), getQueryTimeEnd(request));
    		todo.save();
